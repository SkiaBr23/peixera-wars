...
/*--------------------------------------------------------------------------------------------------------------------------------*/
/*Funções de teste para a engine*/

void teste_cria_estruturas (void){
	CabecaGeral* p = CriaCabecaGeral();
	CriaEstruturas(p);
	
	CU_ASSERT_PTR_NOT_NULL (p);
	CU_ASSERT_PTR_NOT_NULL (p->castle);
	CU_ASSERT_EQUAL (p->castle->ouro,100);
	CU_ASSERT_PTR_NULL (p->castle->quartel);
	CU_ASSERT_PTR_NULL (p->castle->campodetiro);
	CU_ASSERT_PTR_NULL (p->castle->casadaslancas);
	CU_ASSERT_PTR_NULL (p->castle->comercio);
	CU_ASSERT_PTR_NOT_NULL (p->character);
	CU_ASSERT_PTR_NULL (p->character->player);
	CU_ASSERT_PTR_NULL (p->character->cpu);
	
}

void teste_inicializa_estruturas (void){
	CabecaGeral* p = CriaCabecaGeral();
	CriaEstruturas(p);
	InicializaEstruturas(p,1,25,1,50,2,100,3,200,1);
	
	CU_ASSERT_EQUAL(p->castle->ouro,100)
	CU_ASSERT_EQUAL(p->castle->quartel->custounidade,25);
	CU_ASSERT_EQUAL(p->castle->quartel->nivel,1);
	CU_ASSERT_EQUAL(p->castle->campodetiro->custounidade,50);
	CU_ASSERT_EQUAL(p->castle->campodetiro->nivel,2);
	CU_ASSERT_EQUAL(p->castle->casadaslancas->custounidade,100);
	CU_ASSERT_EQUAL(p->castle->casadaslancas->nivel,3);
	CU_ASSERT_EQUAL(p->castle->comercio->taxaouro,200);
	CU_ASSERT_EQUAL(p->castle->comercio->nivel,1);

}

void teste_verifica_interface (void) {
	int i;
	CabecaGeral* p = CriaCabecaGeral();
	CriaEstruturas(p);
	InicializaEstruturas(p,0,25,1,25,1,25,1,100,1);
	
	i = VerificaInterface(p);
	CU_ASSERT_EQUAL(i, 1);
}

void teste_startgame (void){
	int i;
	CabecaGeral* p;
	...
	CU_ASSERT_PTR_NOT_NULL (p);
	CU_ASSERT_PTR_NOT_NULL (p->castle);
	CU_ASSERT_PTR_NOT_NULL (p->castle->quartel);
	CU_ASSERT_PTR_NOT_NULL (p->castle->campodetiro);
	CU_ASSERT_PTR_NOT_NULL (p->castle->casadaslancas);
	CU_ASSERT_PTR_NOT_NULL (p->castle->comercio);
	CU_ASSERT_PTR_NOT_NULL (p->character);
	CU_ASSERT_PTR_NULL (p->character->player);
	CU_ASSERT_PTR_NULL (p->character->cpu);
	...
	CU_ASSERT_EQUAL(p->castle->quartel->custounidade,25);
	CU_ASSERT_EQUAL(p->castle->quartel->nivel,1);
	CU_ASSERT_EQUAL(p->castle->campodetiro->custounidade,25);
	CU_ASSERT_EQUAL(p->castle->campodetiro->nivel,1);
	CU_ASSERT_EQUAL(p->castle->casadaslancas->custounidade,25);
	CU_ASSERT_EQUAL(p->castle->casadaslancas->nivel,1);
	CU_ASSERT_EQUAL(p->castle->comercio->taxaouro,100);
	CU_ASSERT_EQUAL(p->castle->comercio->nivel,1);
