...
/*	--------------------FUNÇÕES DA ENGINE--------------------	*/
/*
Função de inicialização para um novo jogo.
REQUISITO: Ter memória livre.
HIPÓTESE: A função chama a criação de um novo jogo, a partir de valores iniciais padrão.
*/
void Start(){
	CabecaGeral* p;
...
void Inicia(){
	...
	switch(opcao){
		...
		default:
			printf("\nOpcao invalida. Selecione um valor entre 1 e 3.\n");
			Inicia();
	}
}