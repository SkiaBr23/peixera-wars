...
int main () {
	...
	if ((NULL == CU_add_test (suite_testes_unidade, "Teste de criação de unidade", teste_cria_unidade)) ||
	    ...
	    (NULL == CU_add_test (suite_testes_unidade, "Teste de consistência da estrutura Unidade", teste_verifica_estrutura)) ||
	    (NULL == CU_add_test (suite_testes_unidade, "Teste de remoção de unidade", teste_remover_unidade)) ||
	    (NULL == CU_add_test (suite_testes_unidade, "Teste de remoção de unidade no final da lista", teste_remover_unidade_final)) ||
	    (NULL == CU_add_test (suite_testes_unidade, "Teste de limpeza de lista de unidades", teste_limpa_lista))) {
		CU_cleanup_registry();
		return CU_get_error();
	}
