...
void teste_cria_controlador(void){
	...
	CU_ASSERT_PTR_NULL(p->character);
	CU_ASSERT_PTR_NULL(p->castle);
}
...
void teste_cria_cabeca_filas (void) {
	...
	CU_ASSERT_PTR_NOT_NULL (p);
	CU_ASSERT_PTR_NULL (p->player);
	CU_ASSERT_PTR_NULL (p->cpu);
}
...
void teste_cria_castelo (void) {
	...
	CU_ASSERT_PTR_NOT_NULL (p);
	CU_ASSERT_PTR_NULL (p->quartel);
	CU_ASSERT_PTR_NULL (p->casadaslancas);
	CU_ASSERT_PTR_NULL (p->campodetiro);
	CU_ASSERT_PTR_NULL (p->comercio);
	CU_ASSERT_EQUAL (p->ouro, 100);
}
...
void teste_verifica_castelo_interno (void) {
	...
	p->quartel = InicializarEdificio (p->quartel, 1, 1);
	p->casadaslancas = InicializarEdificio (p->casadaslancas, 1, 1);
	p->campodetiro = InicializarEdificio (p->campodetiro, 1, 1);
...
void teste_remover_unidade (void) {
	Unidade * p;
	Unidade * t;
	p = CriaUnidade ();
	
	p = InserirUnidadeFinal (p, 200, 60, 0.2, 2,1);
	p = InserirUnidadeFinal (p, 150, 45, 0.2, 3,1);
	p = InserirUnidadeFinal (p, 30, 67, 0.1, 3,1);

	p = RemoverUnidade (p, p->prox);

	int i = 0;

	for (t = p; t != NULL; t = t->prox) {
		if (i == 0) {
			CU_ASSERT_EQUAL (t->vida, 200);
		}
		if (i == 1) {
			CU_ASSERT_EQUAL (t->vida, 30);
		}
		i++;
	}
}

void teste_remover_unidade_final (void) {
	Unidade * p;
	Unidade * t;
	p = CriaUnidade ();
	
	p = InserirUnidadeFinal (p, 100, 55, 0.2, 2,1);
	p = InserirUnidadeFinal (p, 150, 30, 0.2, 3,1);
	p = InserirUnidadeFinal (p, 300, 90, 0.1, 3,1);

	p = RemoverUnidadeFinal (p);

	p = InserirUnidadeFinal (p, 200, 65, 0.2, 2,1);

	int i = 0;

	for (t = p; t != NULL; t = t->prox) {
		if (i == 0) {
			CU_ASSERT_EQUAL (t->vida, 100);
		}
		if (i == 1) {
			CU_ASSERT_EQUAL (t->vida, 150);
		}
		if (i == 2) {
			CU_ASSERT_EQUAL (t->vida, 200);
		}
...
void teste_limpa_lista (void) {
	Unidade * p;
	p = CriaUnidade ();

	p = InserirUnidadeFinal (p, 100, 60, 0.2, 2,1);
	p = InserirUnidadeFinal (p, 200, 70, 0.2, 3,1);
	p = InserirUnidadeFinal (p, 300, 80, 0.1, 3,1);
