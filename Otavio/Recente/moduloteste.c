...
int main () {
	...
	CU_pSuite suite_testes_engine = NULL;
	...
	/*Adição de uma suite de testes à suite de engine*/
	suite_testes_engine = CU_add_suite ("Testes de Engine", 0, 0);
	if (NULL == suite_testes_engine) {
		CU_cleanup_registry();
		return CU_get_error();
	}
	
	/*Adição dos testes à suite de testes estruturas*/
	if ((NULL == CU_add_test (suite_testes_engine, "Teste de verificação da criação de estruturas", teste_cria_estruturas)) || (NULL == CU_add_test (suite_testes_engine, "Teste de verificação para inicialização das estruturas", teste_inicializa_estruturas)) || (NULL == CU_add_test (suite_testes_engine, "Teste de verificação da interface", teste_verifica_interface)) || (NULL == CU_add_test (suite_testes_engine, "Teste da função StartGame", teste_startgame))) {
		CU_cleanup_registry();
		return CU_get_error();
	}
