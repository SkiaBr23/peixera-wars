...
*/
CabecaGeral* StartGame(CabecaGeral* Cabeca, int horda){	/*horda tem que manipular, ver isso depois*/
...
void Run (CabecaGeral* Interface, int new, int wave){
	...
	if(!CabecaGeralVazia(Interface)){
		...
		while(fim==0 && (horda>0 && horda<=10)){	/*contagem de hordas*/
			...
			if(newhorda=0){	/*momento em que o combate se inicia*/
				/*RAND PARA ESQUIVA*/
				/*RAND PARA DANO*/
			}
			
			while(GanhouBatalha==1 && EvoluiuEdificio==0){	/*a partir da segunda horda já é possível fazer evoluções dos edifícios*/
